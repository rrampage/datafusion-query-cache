...
use datafusion_query_cache::{normalize_temporal_bounds_in_expr, with_query_cache_log, CachedAggregateExec, CacheUpdateAggregateExec, LogStderrColors, MemoryQueryCache, QueryCacheConfig, TimeInterval};
...
fn collect_exec_intervals(plan: &Arc<dyn datafusion::physical_plan::ExecutionPlan>) -> (Vec<TimeInterval>, Vec<TimeInterval>) {
    let mut cached_intervals = Vec::new();
    let mut gap_intervals = Vec::new();

    // Walk the plan tree to find execution nodes
    fn walk_plan(plan: &Arc<dyn datafusion::physical_plan::ExecutionPlan>, cached: &mut Vec<TimeInterval>, gaps: &mut Vec<TimeInterval>) {
...
async fn test_full_miss_then_hit() {
    ...
    let sql = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:00:00Z' AND timestamp < '2024-01-01T01:00:00Z'";
...
async fn test_partial_hit_with_gaps() {
    ...
    let sql_cache = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:30:00Z' AND timestamp < '2024-01-01T00:45:00Z'";
    let _cache_results = execute_query(&ctx, sql_cache).await;

    // Now query a larger interval - should work correctly
    let sql_query = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:00:00Z' AND timestamp < '2024-01-01T01:00:00Z'";
...
async fn test_between_semantics() {
    ...
    let sql_between = "SELECT count(*) FROM records WHERE timestamp BETWEEN '2024-01-01T00:00:00Z' AND '2024-01-01T01:00:00Z'";
...
async fn test_multiple_overlapping_intervals() {
    ...
    let sql1 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:10:00Z' AND timestamp < '2024-01-01T00:40:00Z'";
    let _results1 = execute_query(&ctx, sql1).await;

    let sql2 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:20:00Z' AND timestamp < '2024-01-01T00:50:00Z'";
...
async fn test_param_reuse_across_different_ranges() {
    ...
    let sql_cache = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:04:00Z' AND timestamp < '2024-01-01T00:06:00Z'";
    ...
    let sql_different = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:07:00Z' AND timestamp < '2024-01-01T00:09:00Z'";
    ...
    assert_eq!(cached_intervals.len(), 1, "Expected 1 cached interval for repeated query, got {:?}", cached_intervals);
    assert_eq!(cached_intervals[0], TimeInterval::new(1704067440000000000, 1704067560000000000)); // [4,6) in nanoseconds
    ...
    let count_value = results_same[0].column(0).as_any().downcast_ref::<datafusion::arrow::array::Int64Array>()
        .unwrap().value(0);
...
async fn test_complex_overlapping_intervals() {
    ...
    let sql1 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:01:00Z' AND timestamp < '2024-01-01T00:05:00Z'";
    let _results1 = execute_query(&ctx, sql1).await;

    let sql2 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:02:00Z' AND timestamp < '2024-01-01T00:04:00Z'";
    let _results2 = execute_query(&ctx, sql2).await;

    let sql3 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:03:00Z' AND timestamp < '2024-01-01T00:06:00Z'";
    let _results3 = execute_query(&ctx, sql3).await;

    // Query the union of all intervals [1,6) - should merge all cached data
    let sql_query = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:01:00Z' AND timestamp < '2024-01-01T00:06:00Z'";
    let results = execute_query(&ctx, sql_query).await;
...
async fn test_nested_intervals() {
    ...
    let sql_large = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:01:00Z' AND timestamp < '2024-01-01T00:07:00Z'";
    ...
    let sql_nested = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:02:00Z' AND timestamp < '2024-01-01T00:05:00Z'";
    let results_nested = execute_query(&ctx, sql_nested).await;

    // Verify [2,5) is cached
    let df_nested = ctx.sql(sql_nested).await.unwrap();
    let plan_nested = ctx.state().create_physical_plan(df_nested.logical_plan()).await.unwrap();
    ...
    assert_eq!(large_interval, nested_interval, "Both queries should find the same cached [1,7) interval");
    
    // Now query the large interval again - should use cached [1,7), not [2,5)
    let df_large_again = ctx.sql(sql_large).await.unwrap();
    let physical_plan = ctx.state().create_physical_plan(df_large_again.logical_plan()).await.unwrap();
    ...
    assert_eq!(gap_intervals.len(), 0, "Expected 0 gap intervals - should use complete cached [1,7)");
    
    // Verify it's using the large interval, not the nested one
    assert_eq!(cached_intervals[0], large_interval, "Should use the large [1,7) interval, not [2,5)");

    let results_large2 = df_large_again.collect().await.unwrap();
    assert_eq!(results_large, results_large2, "Results should be identical for same query");

    // Verify nested results are subset (smaller count)
    let count_large = results_large[0].column(0).as_any().downcast_ref::<datafusion::arrow::array::Int64Array>().unwrap().value(0);
    let count_nested = results_nested[0].column(0).as_any().downcast_ref::<datafusion::arrow::array::Int64Array>().unwrap().value(0);
    assert!(count_nested <= count_large, "Nested interval should have smaller or equal count");
}
...
async fn test_adjacent_intervals() {
    ...
    let sql1 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:01:00Z' AND timestamp < '2024-01-01T00:03:00Z'";
    let _results1 = execute_query(&ctx, sql1).await;

    let sql2 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:03:00Z' AND timestamp < '2024-01-01T00:05:00Z'";
    let _results2 = execute_query(&ctx, sql2).await;

    // Query spanning both intervals: [1,5) - should merge adjacent intervals
    let sql_query = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:01:00Z' AND timestamp < '2024-01-01T00:05:00Z'";
    let results = execute_query(&ctx, sql_query).await;
...
async fn test_multiple_disjoint_intervals() {
    ...
    let sql1 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:01:00Z' AND timestamp < '2024-01-01T00:02:00Z'";
    let _results1 = execute_query(&ctx, sql1).await;

    let sql2 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:04:00Z' AND timestamp < '2024-01-01T00:05:00Z'";
    let _results2 = execute_query(&ctx, sql2).await;

    let sql3 = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:07:00Z' AND timestamp < '2024-01-01T00:08:00Z'";
    let _results3 = execute_query(&ctx, sql3).await;
    ...
    assert_eq!(cached_intervals.len(), 1, "Expected 1 cached interval for repeated query, got {}", cached_intervals.len());
...
async fn test_partial_coverage_multiple_disjoint() {
    ...
    let sql_cache = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:02:00Z' AND timestamp < '2024-01-01T00:03:00Z'";
    let _results_cache = execute_query(&ctx, sql_cache).await;


    // Query a larger interval that overlaps with the cached one: [1,4)
    // This should use [2,3) and compute [1,2) and [3,4)
    let sql_query = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:01:00Z' AND timestamp < '2024-01-01T00:04:00Z'";
    let df = ctx.sql(sql_query).await.unwrap();
    let physical_plan = ctx.state().create_physical_plan(df.logical_plan()).await.unwrap();
    let (cached_intervals, gap_intervals) = collect_exec_intervals(&physical_plan);

    // Should use the cached interval [2,3) that overlaps with [1,4)
    assert_eq!(cached_intervals.len(), 1, "Expected 1 cached interval, got {}", cached_intervals.len());
    assert_eq!(cached_intervals[0], TimeInterval::new(1704067320000000000, 1704067380000000000), "Should use cached [2,3) interval");

    // Should have gaps for [1,2) and [3,4)
    assert_eq!(gap_intervals.len(), 2, "Expected 2 gap intervals, got {}", gap_intervals.len());
    assert_eq!(gap_intervals[0], TimeInterval::new(1704067260000000000, 1704067320000000000), "First gap should be [1,2)");
    assert_eq!(gap_intervals[1], TimeInterval::new(1704067380000000000, 1704067440000000000), "Second gap should be [3,4)");
...
async fn test_non_overlapping_close_intervals() {
    ...
    let sql_cache = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:01:00Z' AND timestamp < '2024-01-01T00:02:00Z'";
    let _results_cache = execute_query(&ctx, sql_cache).await;

    // Query a different interval: [3,4) - should not use cached data
    let sql_query = "SELECT count(*) FROM records WHERE timestamp >= '2024-01-01T00:03:00Z' AND timestamp < '2024-01-01T00:04:00Z'";
    let df = ctx.sql(sql_query).await.unwrap();
    let physical_plan = ctx.state().create_physical_plan(df.logical_plan()).await.unwrap();
    let (cached_intervals, gap_intervals) = collect_exec_intervals(&physical_plan);

    // Should not use cached data (different interval)
    assert_eq!(cached_intervals.len(), 0, "Expected 0 cached intervals for different range, got {}", cached_intervals.len());
    assert!(gap_intervals.len() >= 1, "Expected at least 1 gap interval, got {}", gap_intervals.len());
